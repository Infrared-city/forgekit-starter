...
function readEnv(env, key) {
  ...
  return _optionalChain([proc, 'optionalAccess', _ => _.env, 'optionalAccess', _2 => _2[key]]);
}
...
function zipPayloadJson(jsonBytes) {
  return _fflate.zipSync.call(void 0, { "payload.json": jsonBytes }, { level: 6 });
}
...
async function postEnvelope(args) {
  ...
  try {
    response = await fetch(args.url, {
      ...
      headers: {
        ...args.authHeaders,
        ..._nullishCoalesce(args.extraHeaders, () => ( {})),
...
async function presignUpload(args) {
  ...
  try {
    ...
    if (err instanceof Error && err.name === "AbortError") {
      throw new (0, _chunkROJ27LGGcjs.BigPayloadPresignError)(
  ...
  try {
    ...
    throw new (0, _chunkROJ27LGGcjs.BigPayloadPresignError)("Presign response was not valid JSON", {
      statusCode: response.status,
      responseBody: text
    });
  }
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new (0, _chunkROJ27LGGcjs.BigPayloadPresignError)("Presign response JSON was not an object", {
      statusCode: response.status,
      responseBody: text
    });
  }
...
async function preflight(url, redacted, signal) {
  ...
  if (contentRange) {
    ...
    if (m && Number(m[1]) > RESPONSE_MAX_BYTES) {
      throw new (0, _chunkROJ27LGGcjs.BigPayloadFetchError)(
        "REF_TOO_LARGE",
...
function decodeInner(zipBytes, redacted) {
  let files;
  try {
    files = _fflate.unzipSync.call(void 0, zipBytes);
  } catch (err) {
    throw new (0, _chunkROJ27LGGcjs.BigPayloadFetchError)(
      "REF_DECODE_FAILED",
  ...
  const inner = _nullishCoalesce(files[PAYLOAD_ENTRY], () => ( (firstFile ? files[firstFile] : void 0)));
  if (!inner) {
    throw new (0, _chunkROJ27LGGcjs.BigPayloadFetchError)(
      "REF_DECODE_FAILED",
...
async function getNoAuth(url, headers, timeoutMs, redacted, signal) {
  ...
  try {
    ...
    if (_optionalChain([signal, 'optionalAccess', _3 => _3.aborted])) throw err;
    throw new (0, _chunkROJ27LGGcjs.BigPayloadFetchError)(
      "REF_FETCH_TIMEOUT",
...
function checkContentType(resp, redacted) {
  ...
  const primary = (_nullishCoalesce(ct, () => ( ""))).split(";", 1)[0].trim().toLowerCase();
...
async function putZipToS3(uploadUrl, zipBytes) {
  ...
  if (lastStatus !== void 0) {
    if (lastBody.includes("SignatureDoesNotMatch")) {
      throw new (0, _chunkROJ27LGGcjs.BigPayloadUploadError)(
    ...
    throw new (0, _chunkROJ27LGGcjs.BigPayloadUploadError)(`S3 PUT failed: ${lastStatus}`, {
      statusCode: lastStatus,
      responseBody: lastBody
    });
  }
  throw new (0, _chunkROJ27LGGcjs.BigPayloadUploadError)(
    `S3 PUT failed at transport layer: ${_nullishCoalesce(lastTransport, () => ( "unknown"))}`
  );
}
...
async function postWithBigPayload(args) {
  ...
  if (!useEnvelope) {
    _optionalChain([args, 'access', _4 => _4.logger, 'optionalAccess', _5 => _5.debug, 'call', _6 => _6({
  ...
  _optionalChain([args, 'access', _7 => _7.logger, 'optionalAccess', _8 => _8.info, 'call', _9 => _9({
...
async function postZipWithBigPayload(args) {
  ...
  if (!useEnvelope) {
    _optionalChain([args, 'access', _10 => _10.logger, 'optionalAccess', _11 => _11.debug, 'call', _12 => _12({
  ...
  _optionalChain([args, 'access', _13 => _13.logger, 'optionalAccess', _14 => _14.info, 'call', _15 => _15({
...
async function runEnvelopeFlow(args) {
  ...
  for (let attempt = 1; attempt <= MAX_REF_EXPIRED_RETRIES; attempt++) {
    ...
    if (outcome.code !== "REF_EXPIRED") {
      throw new (0, _chunkROJ27LGGcjs.BigPayloadFetchError)(outcome.code, outcome.message, {
        statusCode: response.status,
        responseBody: response.bodyText
      });
    }
    _optionalChain([args, 'access', _16 => _16.logger, 'optionalAccess', _17 => _17.warn, 'call', _18 => _18({
  ...
  if (finalOutcome.code === "REF_EXPIRED") {
    throw new (0, _chunkROJ27LGGcjs.RefExpiredRetryExhausted)(
  ...
  throw new (0, _chunkROJ27LGGcjs.BigPayloadFetchError)(finalOutcome.code, finalOutcome.message, {
    statusCode: response.status,
    responseBody: response.bodyText
  });
}
...
exports.readEnabled = readEnabled; exports.readThresholdBytes = readThresholdBytes; exports.deriveGatewayBaseUrl = deriveGatewayBaseUrl; exports.zipPayloadJson = zipPayloadJson; exports.postWithBigPayload = postWithBigPayload; exports.postZipWithBigPayload = postZipWithBigPayload;
//# sourceMappingURL=chunk-RCUR3TGS.cjs.map